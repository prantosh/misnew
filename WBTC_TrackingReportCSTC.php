...
$querycc = "SELECT * from cstcmis.veh0214 a,cstcmis.unit b where a.depot = b.UNIT and a.runfleet = 'R' order by a.vehno";
		...
		<div id="content" class="content">
			...
			<div class="row">
			    ...
			    <div class="col-md-12">
                  ...
                  <th COLSPAN="2">1ST SHIFT </th>
                  <th COLSPAN="2">2ND SHIFT </th>
...
while ($rowcc=mysqli_fetch_array($resultcc) ){ 
  // if($rowcc['sch_km'] > 0 && $rowcc['hsd'] > 0){
                ...
                //$querycc = "SELECT * from cstcmis.maint_tran_mobile where maint_date = DATE_FORMAT(DATE_ADD(NOW(), INTERVAL '12:30' HOUR_MINUTE),'%Y-%m-%d') and depot = '" . $_SESSION['UNIT'] . "' order by vehno";
                $name = '';
                $querymob = "SELECT a.shift shift1,a.device device1,a.route route1,b.NAME NAME1 from cstcmis.maint_tran_mobile a,cstcmis.cstc_user b where a.op_code = b.USER_ID AND a.maint_date = DATE_FORMAT(DATE_ADD(NOW(), INTERVAL '12:30' HOUR_MINUTE),'%Y-%m-%d') and a.vehno = '" . $rowcc['vehno'] . "'";
                $resultmob = mysqli_query($cstccon,$querymob) or die(mysqli_error());
                if(mysqli_num_rows($resultmob) > 0){
                while ($rowmob=mysqli_fetch_array($resultmob) ){ 
                    
                    if($rowmob['shift1'] == '1' && $rowmob['device1'] == 'M'){
                        $device_1 = 'MOBILE';
                    }
                    if($rowmob['shift1'] == '1' && $rowmob['device1'] == 'V'){
                        $device_1 = 'VTU';
                    }
                    if($rowmob['shift1'] == '1'){
                        $route_1 = $rowmob['route1'];
                    }
                    if($rowmob['shift1'] == '2' && $rowmob['device1'] == 'M'){
                        $device_2 = 'MOBILE';
                    }
                    if($rowmob['shift1'] == '2' && $rowmob['device1'] == 'V'){
                        $device_2 = 'VTU';
                    }
                    if($rowmob['shift1'] == '2'){
                        $route_2 = $rowmob['route1'];
                    }
                $name =   $rowmob['NAME1'];  
                }
                }
