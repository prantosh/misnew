...
class DB_mysqli extends DB_common
{
    ...
    function simpleQuery($query)
    {
        ...
        if (!$this->autocommit && $ismanip) {
            if ($this->transaction_opcount == 0) {
                $result = @mysqli_query($cstccon,$this->connection, 'SET AUTOCOMMIT=0');
                $result = @mysqli_query($cstccon,$this->connection, 'BEGIN');
                if (!$result) {
        ...
        $result = @mysqli_query($cstccon,$this->connection, $query);
        if (!$result) {
            return $this->mysqliRaiseError();
        }
        if (is_object($result)) {
    ...
    function commit()
    {
        if ($this->transaction_opcount > 0) {
            ...
            $result = @mysqli_query($cstccon,$this->connection, 'COMMIT');
            $result = @mysqli_query($cstccon,$this->connection, 'SET AUTOCOMMIT=1');
    ...
    function rollback()
    {
        if ($this->transaction_opcount > 0) {
            ...
            $result = @mysqli_query($cstccon,$this->connection, 'ROLLBACK');
            $result = @mysqli_query($cstccon,$this->connection, 'SET AUTOCOMMIT=1');
            $this->transaction_opcount = 0;
            if (!$result) {
                return $this->mysqliRaiseError();
    ...
    function tableInfo($result, $mode = null)
    {
        if (is_string($result)) {
             ...
             */
            $id = @mysqli_query($cstccon,$this->connection,
