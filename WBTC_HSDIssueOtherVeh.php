...
$sql1="insert into cstc_store.hsd_issue_to_other_vehicle(DEPOT,VEHNO,DOI,QTY,UPDUSR,TRAN_TYPE) VALUES('" . $unit . "','" . $veh_no . "','" . $issue_date3 . "'," . $qty . ",'" . $user_id . "','I')";
$result1=mysqli_query($cstccon,$sql1);

$sql2="update cstc_store.bincrd_depot set ISS_QTY = ISS_QTY + " . $qty . " where FIN_YR = '" . $CUR_FIN_YR . "' AND DEPOT = '" . $unit . "' AND PART_NO = 'HSD'";
$result2=mysqli_query($cstccon,$sql2);

$query = "SELECT * FROM cstc_store.bincrd_depot where FIN_YR = '" . $CUR_FIN_YR . "' AND DEPOT = '" . $unit . "' AND PART_NO = 'HSD'";
$Recordset = mysqli_query($cstccon,$query) or die(mysqli_error());
$row = mysqli_fetch_assoc($Recordset);
$hsd_stock = $row['OPNG_QTY'] + $row['RCT_QTY'] - $row['ISS_QTY'];

$sql21="update cstc_store.unit set hsd_stock = " . $hsd_stock . " where UNIT = '" . $unit . "'";
